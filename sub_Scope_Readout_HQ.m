...
if params.Scope.ArmTrigger;
    ...
    triggered = str2double(query(params.Scope.visaObj, ':TER?'));
    waitcount = 0;
    while ~triggered
      pause(1);
      triggered = str2double(query(params.Scope.visaObj,':TER?'));
...
if params.SG.Waveform.repeats > 1
   segs = 1:params.SG.Waveform.repeats; 
    
   allsegmentsread = (str2double(query(params.Scope.visaObj, ':WAVeform:SEGMented:COUNt?')) == params.SG.Waveform.repeats);
   while ~allsegmentsread
       pause(0.25);
       allsegmentsread = (str2double(query(params.Scope.visaObj, ':WAVeform:SEGMented:COUNt?')) == params.SG.Waveform.repeats);
   end
...
waveform.XData = [];%zeros(maxdatapts/nsegs, nsegs);
waveform.YData = [];%zeros(maxdatapts/nsegs, nsegs);

for seg = segs

tdel = 0;
if numel(segs) > 1
    fprintf(params.Scope.visaObj, ':ACQuire:SEGMented:INDex %1.0f', seg);
    tdel = str2double(query(params.Scope.visaObj, ':WAVeform:SEGMented:TTAG?'));
end
...
% Generate X & Y Data
waveform.XData(:,seg) = (waveform.XIncrement.*(1:length(waveform.RawData))') - waveform.XIncrement + waveform.XOrigin + tdel;
waveform.YData(:,seg) = (waveform.YIncrement.*(waveform.RawData - waveform.YReference)) + waveform.YOrigin; 
